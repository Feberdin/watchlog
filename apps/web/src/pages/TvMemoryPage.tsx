            ...
            <table className="min-w-full divide-y divide-slate-800 text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                  ...
                  <tr key={item.id} className="align-top">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <p className="mt-1 text-slate-400">
                        {[item.year ?? "ohne Startjahr", metadataLabel(item.genres), `TMDb ${item.tmdbId ?? "-"}`, item.voteAverage ? `${item.voteAverage.toFixed(1)}/10` : null].filter(Boolean).join(" · ")}
                      </p>
                ...
                {filteredItems.length === 0 && (
                  <tr>
                    <td className="px-4 py-6 text-center text-slate-400" colSpan={4}>Keine offene Serie passt zum gewählten Genre.</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            {archivedItems.map((item) => (
              <article key={`${item.id}-${item.status}`} className="flex items-center gap-3 rounded-md border border-slate-800 bg-slate-950 p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-slate-400">{[item.year ?? "ohne Startjahr", metadataLabel(item.genres), actionLabel(item.status)].filter(Boolean).join(" · ")}</p>
                </div>
              </article>
            ))}
          </div>
